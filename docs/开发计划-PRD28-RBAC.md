...
1. 本计划版本号基于 **V101**，RBAC 占 **V102–V110**（V103 为卡片4 种子修复，后续顺延；**V109 于 2026-09-11 被模块菜单管理增强需求占用**——enabled 开关+自定义目录，见《优化记录》同日条目，卡片12 清理卡顺延 V110；同日跟进修复 fix/menu-manage-followup：按钮裁切、空目录删除放宽（子树无 PAGE 即可删，含内置；无迁移））；开工任一带迁移的分支前必须重新确认最大号，被占则顺延并改《方案》+本文件；
...
- [x] 准备上线 SQL 核对脚本（各表行数、管理员权限数、孤儿=0）
