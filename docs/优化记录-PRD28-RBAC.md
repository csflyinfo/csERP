...
### 2026-09-13 交互修复：侧边栏目录点击只展开/收起，不再自动打开第一个页面（fix/sidebar-dir-toggle，无迁移）

> 用户反馈（原话）：「如果点击目录菜单、展开下级菜单。点击已展开菜单收起。点击上级菜单不要自动打开第个目录页面、只有点击页面菜单才打开相应页面。」

- 根因/原状：`AppShell.vue#onRootClick` 对一级目录是「展开 + navigate(firstPageCode(root))」两件事一起做（PRD-28 卡片8 三级菜单时为兼容一级页面保留的行为）；二级目录 `toggleDir` 本来就只展开/折叠。用户要求目录与页面的点击语义彻底分开。
- 改动（纯前端，仅 `frontend/src/layout/AppShell.vue`）：
  1. `onRootClick(root)`：目录（menuType=DIR）只切换展开——当前已展开则显式全部收起（rootOverride=''），否则手风琴切到该根；**不再导航**。一级本身是页面（V109 允许的 L1 挂页，menuType=PAGE）时才 navigate 自身。
  2. 展开态模型从 `manualRoot`（字符串、与路由态互相覆盖不清）改为 `rootOverride`：`null`=跟随当前路由所属根（进页面/顶栏切 tab 自动展开对应目录），具体 code=手动展开，`''`=显式全部收起；`navigate()` 成功打开页面时把覆盖清回 null，保证切模块后展开态正确。
  3. 一级目录头加 ▸/▾ 展开箭头（右侧；选中渐变底上浅色），title 提示「点击展开/收起」；模板由 rootCode 反复 find 改为直接 `v-for="root in menuStore.tree"`；长目录名省略号。二级目录（财务管理 > 总账，三级页面 16 项）维持原 toggle 语义。
  4. 快捷搜索 `quickLocate` 去掉对 manualRoot 的写入：搜到目录进其第一个页面、搜到页面直接打开（搜索是"我要去某个功能"，保留直达），展开态交由路由反算。
- 验证（隔离 8081 全新 H2 库 + 5174 + Edge headless CDP，未碰用户进程；临时库/profile 用后即删）：`npm run build` 通过；CDP 9/9——登录后默认展开当前根；点已展开根→子菜单消失、箭头翻 ▸、路径不变；再点→展开、箭头翻 ▾、路径不变；点另一个根→手风琴单开、路径不变；财务管理>总账 二级目录点击展开 16 个三级页/再点收起、全程路径不变；点「商品档案」页面→/goods 打开、所属根保持展开、页面高亮；在 /goods 上收起目录→页面路由保留、再展开页面仍高亮；控制台零异常。
