...
async function main() {
  ...
  await post('/base/goods/update', { goodsCode, goodsName: '自动化商品-编辑', spec: '1*12', categoryName: '测试分类', baseUnit: '箱', standardPrice: 77, goodsType: '正常商品', shelfLifeDays: 120, storageProperty: '常温', canReturn: true })
  const updatedGoods = await post('/base/goods/page', { pageNo: 1, pageSize: 20, filters: { keyword: goodsCode } })
  assert(updatedGoods.records.some(record => record.goodsCode === goodsCode && record.goodsName === '自动化商品-编辑' && Number(record.standardPrice) === 77), 'updated goods should be persisted')
  await post('/base/goods/freeze', { bizId: goodsCode })
  const frozenGoods = await post('/base/goods/page', { pageNo: 1, pageSize: 20, filters: { keyword: goodsCode } })
  assert(frozenGoods.records.some(record => record.goodsCode === goodsCode && record.status === '冻结'), 'frozen goods should persist status')
  await post('/base/goods/stop', { bizId: goodsCode })
  const stoppedGoods = await post('/base/goods/page', { pageNo: 1, pageSize: 20, filters: { keyword: goodsCode } })
  assert(stoppedGoods.records.some(record => record.goodsCode === goodsCode && record.status === '停用'), 'stopped goods should persist status')
  ...
  await post('/base/master/freeze', { moduleCode: 'customer', bizId: customerCode })
  const frozenCustomers = await post('/base/master/customer/page', { pageNo: 1, pageSize: 20, filters: { keyword: customerCode } })
  assert(frozenCustomers.records.some(record => record.customerCode === customerCode && record.status === '冻结'), 'frozen customer should persist status')
  await post('/base/master/save', { moduleCode: 'customer', customerId: customerCode, customerCode, customerName: '自动化客户编辑', channelType: '批发' })
  const editedFrozenCustomers = await post('/base/master/customer/page', { pageNo: 1, pageSize: 20, filters: { keyword: customerCode } })
  assert(editedFrozenCustomers.records.some(record => record.customerCode === customerCode && record.status === '冻结'), 'edited frozen customer should keep status')
  await post('/base/master/unfreeze', { moduleCode: 'customer', bizId: customerCode })
  const normalCustomers = await post('/base/master/customer/page', { pageNo: 1, pageSize: 20, filters: { keyword: customerCode } })
  assert(normalCustomers.records.some(record => record.customerCode === customerCode && record.status === '正常'), 'unfrozen customer should return to normal')
