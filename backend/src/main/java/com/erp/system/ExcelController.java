    ...
    private List<List<Object>> buildBody(List<Map<String, Object>> data, String moduleCode) {
        ...
        for (Map<String, Object> row : data) {
            result.add(switch (moduleCode) {
                ...
                case "goods" -> Arrays.asList(row.get("goods_code"), row.get("goods_name"), row.get("spec"), row.get("category_name"), row.get("brand_name"), row.get("base_unit"), row.get("barcode"), row.get("standard_price"), row.get("latest_purchase_price"), row.get("min_sale_price"), com.erp.base.GoodsBizPolicy.typeLabel(row.get("goods_type") == null ? "" : String.valueOf(row.get("goods_type"))), row.get("shelf_life_days"), row.get("storage_property"), row.get("suggested_retail_price"), row.get("stock_upper_limit"), row.get("stock_lower_limit"), row.get("default_supplier"), row.get("default_warehouse"), row.get("status"));
