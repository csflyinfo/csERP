export const fallbackMenus = {
  首页: [{ code: 'dashboard', name: '经营概览' }],
  基础资料: [
    { code: 'goods', name: '商品档案' },
    { code: 'category', name: '商品分类' },
    { code: 'brand', name: '品牌管理' },
    { code: 'unit', name: '单位管理' },
    { code: 'customer', name: '门店/客户资料' },
    { code: 'supplier', name: '供应商资料' },
    { code: 'warehouse', name: '仓库资料' },
    { code: 'priceGroup', name: '价格组设置' },
    { code: 'priceGroupItem', name: '价格组商品查询' },
    { code: 'priceAdjustOrder', name: '价格组调价单' },
    { code: 'goodsPriceAdjust', name: '商品综合调价单' },
    { code: 'priceChangeLog', name: '商品变价查询' },
    { code: 'customerPrice', name: '客户价格调整单' },
    { code: 'customerPriceQuery', name: '客户价格查询' },
    { code: 'customerPriceChange', name: '客户商品变价查询' },
    { code: 'territory', name: '片区管理' },
    { code: 'routeLine', name: '线路管理' },
    { code: 'employee', name: '人员信息' },
    { code: 'department', name: '部门管理' },
    { code: 'owner', name: '货主信息' },
    { code: 'expenseType', name: '费用类型' },
    { code: 'counterparty', name: '往来单位' },
    { code: 'fundAccount', name: '资金账户' },
  ],
  采购管理: [
    { code: 'purchaseOrder', name: '采购订单' },
    { code: 'purchaseInbound', name: '采购入库' },
    { code: 'purchaseReceipt', name: '采购收货单' },
    { code: 'purchaseReturnApply', name: '采购退货申请' },
    { code: 'purchaseReturnOutbound', name: '采购退货出库' },
    { code: 'purchaseReturn', name: '采购退货单' },
    { code: 'purchaseExpense', name: '采购费用单' },
    { code: 'purchaseInvoice', name: '采购发票' },
  ],
  销售管理: [
    { code: 'quickOrder', name: '销售快速开单' },
    { code: 'salesOrder', name: '销售订单' },
    { code: 'salesOutbound', name: '销售出库' },
    { code: 'salesReceipt', name: '销售发货单' },
    { code: 'rejectInbound', name: '拒收入库单' },
    { code: 'salesReturn', name: '销售退货单' },
    { code: 'salesReturnInbound', name: '销售退货入库' },
    { code: 'salesInvoice', name: '销售发票' },
    { code: 'flyOrder', name: '飞单' },
    { code: 'emptyAdjust', name: '客户空退空出' },
  ],
  库存管理: [
    { code: 'stockBalance', name: '库存查询' },
    { code: 'stockLedger', name: '库存流水' },
    { code: 'stockWarning', name: '库存预警' },
    { code: 'transferApply', name: '调拨申请单' },
    { code: 'transferOutbound', name: '调拨出库单' },
    { code: 'transferInbound', name: '调拨入库单' },
    { code: 'damage', name: '报损单' },
    { code: 'costAdjust', name: '成本调整单' },
    { code: 'stockAdjust', name: '库存调整单' },
    { code: 'otherInbound', name: '其他入库' },
    { code: 'otherOutbound', name: '其他出库' },
    { code: 'stockTake', name: '库存盘点' },
  ],
  财务管理: [
    { code: 'ar', name: '客户应收明细表' },
    { code: 'ap', name: '应付账款' },
    { code: 'receiptPayment', name: '收款单' },
    { code: 'paymentModule', name: '付款单' },
    { code: 'reconcileRecord', name: '收款核销流水表' },
    { code: 'arSettlement', name: '应收结算' },
    { code: 'apSettlement', name: '应付结算' },
    { code: 'financeExpense', name: '费用单' },
    { code: 'fundLedger', name: '资金流水' },
    { code: 'counterpartyAr', name: '往来单位应收' },
    { code: 'counterpartyAp', name: '往来单位应付' },
    { code: 'receiptVerify', name: '收款核销' },
    { code: 'paymentVerify', name: '付款核销' },
    { code: 'customerStatement', name: '客户对账' },
    { code: 'supplierStatement', name: '供应商对账' },
  ],
  总账管理: [
    { code: 'glAccount', name: '会计科目' },
    { code: 'glInitBalance', name: '总账初始化' },
    { code: 'glAuxProject', name: '核算项目' },
    { code: 'glVoucher', name: '凭证管理' },
    { code: 'glEvent', name: '待生成凭证' },
    { code: 'glVoucherTemplate', name: '凭证模板' },
    { code: 'glTransferTemplate', name: '转账模板' },
    { code: 'glBizSubjectMap', name: '业务类型映射' },
    { code: 'glArchiveMapping', name: '档案科目映射' },
    { code: 'glLedger', name: '账簿查询' },
    { code: 'glAssetCard', name: '资产卡片' },
    { code: 'glDepreciation', name: '折旧计提' },
    { code: 'glAssetCheck', name: '资产盘点' },
    { code: 'glPeriodClose', name: '期末处理' },
    { code: 'glReport', name: '总账报表' },
    { code: 'glReconcile', name: '业财对账' },
  ],
  报表中心: [
    { code: 'chartReport', name: '图表报表' },
    { code: 'salesReport', name: '销售报表' },
    { code: 'purchaseReport', name: '采购报表' },
    { code: 'stockReport', name: '库存报表' },
    { code: 'financeReport', name: '财务报表' },
  ],
  系统管理: [
    { code: 'user', name: '用户管理' },
    { code: 'role', name: '权限组管理' },
    { code: 'param', name: '系统参数' },
    { code: 'paramSetting', name: '参数设置' },
    { code: 'billNo', name: '单据编号规则' },
    { code: 'precision', name: '显示精度设置' },
    { code: 'dictionary', name: '用户数据字典' },
    { code: 'workflow', name: '审批流配置' },
    { code: 'printTemplate', name: '打印模板设置' },
    { code: 'importList', name: '导入列表' },
    { code: 'exportCenter', name: '导出中心' },
    { code: 'log', name: '操作日志' },
    { code: 'loginLog', name: '登录日志' },
  ],
  '仓储作业(WMS)': [
    { code: 'wms-dashboard', name: '作业看板' },
    { code: 'wms-preallocation', name: '预分配/下放' },
    { code: 'wms-wave', name: '波次管理' },
    { code: 'wms-pick', name: '拣货任务' },
    { code: 'wms-sort-instruction', name: '分拣指令查询' },
    { code: 'wms-check', name: '复核/打包' },
    { code: 'wms-load', name: '装车/发运' },
    { code: 'wms-inbound', name: '入库任务' },
    { code: 'wms-putaway', name: '上架任务' },
    { code: 'wms-replenish', name: '补货管理' },
    { code: 'wms-move', name: '移库移位' },
    { code: 'wms-adjust', name: '库存调整' },
    { code: 'wms-damage', name: '报损管理' },
    { code: 'wms-freeze', name: '库存冻结' },
    { code: 'wms-assembly', name: '组装拆卸' },
    { code: 'wms-stocktake', name: '盘点管理' },
    { code: 'wms-expiry', name: '效期管理' },
    { code: 'wms-zone', name: '库区管理' },
    { code: 'wms-bin', name: '库位管理' },
    { code: 'wms-collect-zone', name: '集货区管理' },
    { code: 'wms-pick-binding', name: '拣货位商品绑定' },
    { code: 'wms-bin-stock', name: '库位库存' },
    { code: 'wms-stock-query', name: 'WMS库存查询' },
    { code: 'wms-exception', name: '异常中心' },
  ],
  运输管理: [
    { code: 'tms-dispatch-pool', name: '配送任务池' },
    { code: 'tms-dispatch-list', name: '调度单管理' },
    { code: 'tms-return-dispatch', name: '退货单调度' },
    { code: 'tms-delivery-monitor', name: '在途监控' },
    { code: 'tms-sign-verify', name: '签收核销' },
    { code: 'tms-driver-return', name: '司机退货单' },
    { code: 'tms-reschedule-return', name: '改派返仓单' },
    { code: 'tms-customer-reject', name: '客户拒收单' },
    { code: 'tms-exception-report', name: '异常上报处理' },
    { code: 'tms-settlement', name: '交账单管理' },
    { code: 'tms-store-location', name: '门店定位审核' },
    { code: 'tms-dashboard', name: '调度看板' },
  ],
  工作台: [
    { code: 'todo', name: '待办中心' },
    { code: 'notification', name: '消息通知' },
  ],
}
