...
function valueForTitle(title, record, fallback = '') {
  const aliases = [
    ...
    [/(库存金额)/, ['stockAmount']],
    [/(成本单价|成本价)/, ['costPrice']],
    [/(客户编码)/, ['customerCode', 'code']],
    [/(客户名称|客户编号\/名称|客户|门店)/, ['customer', 'customerName', 'objectName', 'name']],
    [/(渠道类型)/, ['channelType']],
    [/(联系人)/, ['contactName']],
    [/(片区)/, ['territory']],
    [/(线路)/, ['routeLine']],
    ...
    [/(发票抬头)/, ['invoiceTitle']],
    [/(税号)/, ['taxNo']],
    [/(供应商编码)/, ['supplierCode', 'code']],
    [/(供应商简称)/, ['shortName']],
    [/(供应商类型|类型)/, ['supplierType', 'type']],
    [/(供应商名称|供应商)/, ['supplier', 'supplierName', 'objectName', 'name']],
    [/(电话)/, ['phone']],
    [/(到货天数)/, ['deliveryDays']],
    ...
    [/(示例)/, ['exampleNo']],
    [/(任务号)/, ['taskNo', 'code']],
    [/(报表名称|任务名称)/, ['reportName', 'taskName', 'name']],
    [/(模块编码)/, ['moduleCode', 'type']],
    [/(文件名)/, ['fileName']],
    [/(成功行数)/, ['successRows']],
    [/(失败行数)/, ['failedRows']],
    [/(结果说明|筛选条件)/, ['resultText', 'filterText', 'remark']],
    [/(创建时间)/, ['createdAt']],
    [/(完成时间)/, ['finishedAt']],
    [/(操作时间)/, ['operateAt']],
    [/(操作人)/, ['operatorName']],
