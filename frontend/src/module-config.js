...
Object.assign(moduleConfigs, {
  user: {
    ...
    row: ['admin', '系统管理员', '13800000000', '管理员组', '全部', '正常', '编辑 停用 日志'],
  ...
  role: {
    ...
    row: ['ADMIN', '管理员组', '1', '全部菜单', '全部字段', '正常', '编辑 复制 停用 用户'],
  ...
  param: {
    ...
    row: ['CREDIT_CHECK_MODE', '信用控制', 'REMIND', 'REMIND', '销售', '可选 REMIND/BLOCK/APPROVAL', '编辑 日志'],
  ...
  billNo: {
    ...
    row: ['销售订单', 'SO', 'yyyyMMdd', '5', 'DAY', 'SO2026061500001', '正常', '编辑 预览 日志'],
  ...
  log: {
    ...
    row: ['2026-06-15 09:00:00', '系统管理员', 'system', 'INIT', 'INIT', 'SUCCESS', '系统初始化', '查看'],
...
Object.assign(moduleConfigs, {
  customerPrice: {
    ...
    row: ['CPA202606140001', 'C001 华联超市', '2026-06-14', '定时生效', '2026-06-15 08:00', '2026-06-15 ~ 2026-12-31', '2', '管理员 10:20', '待审核', '查看 编辑 审核 作废 导入'],
  ...
  customerPriceQuery: {
    ...
    row: ['C001 华联超市', 'SP001', '农夫山泉500ml*24', '瓶', '500ml*24', '6941410749551', '35.00', '34.50', '31.20', '30.80', '定时生效', '长期有效', '生效中', '查看来源 停用'],
...
Object.assign(moduleConfigs, {
  transfer: {
    ...
    row: ['TR202606140001', '2026-06-14', '总仓', '东区仓', '普通调拨', '一步调拨', '100', '0', '3080.00', '待审核', '查看 编辑 审核 出库 入库 关闭 日志'],
  ...
  purchaseExpense: {
    ...
    row: ['PE202606140001', '2026-06-14', '运费', '物流公司', '顺丰物流', '1000.00', '60.00', '1060.00', '1000.00', '0.00', '按金额', '已分摊', '是', '待审核', '查看 编辑 审核 分摊 查看应付'],
  ...
  purchaseInvoice: {
    ...
    row: ['PINV202606140001', '33002606140001', '033002600111', '增值税专票', '农夫山泉杭州经销', '2026-06-14', '3500.00', '455.00', '3955.00', '0.00', '3955.00', '未勾稽', '未认证', '待审核', '查看 编辑 审核 勾稽 认证'],
  ...
  financeExpense: {
    ...
    row: ['FE202606140001', '2026-06-14', '支出', '房租', '供应商', '物业公司', '5000.00', '0.00', '是', 'AP202606140001', '否', '-', '待审核', '查看 编辑 审核 查看往来'],
...
moduleConfigs.ar = {
  ...
  row: ['AR202606140001', 'SR202606140001', '华联超市', '张三', '350.00', '0.00', '350.00', '2026-07-14', '0', '未核销', '未开票', '查看来源 发起收款 应收结算 核销记录'],
}
moduleConfigs.ap = {
  ...moduleConfigs.ap,
  columns: ['应付单号', '来源采购收货单', '供应商', '应付金额', '已付金额', '未付金额', '预计付款日', '逾期天数', '核销状态', '操作'],
  row: ['AP202606140001', 'PR202606140001', '农夫山泉杭州经销', '3955.00', '0.00', '3955.00', '2026-07-14', '0', '未核销', '查看来源 发起付款 应付结算 核销记录'],
...
Object.assign(moduleConfigs, {
  goods: {
    ...
    row: ['-', 'SP001', '农夫山泉500ml*24', '正常商品', '500ml*24', '瓶装水', '农夫山泉', '瓶', '6941410749551', '365天', '常温', '35.00', '39.00', '31.20', '30.00', '2000', '200', '农夫山泉杭州经销', '总仓', '是/是/是', '1200', '正常', '编辑 复制 停用 删除 库存 历史'],
    actions: ['新增商品', '批量编辑', '导入商品', '导出', '删除'],
  ...
  customer: {
    ...
    row: ['C001', '华联超市', '零售商超', '王店长', '138****8888', '西湖区', '朝阳线', '张三', '金牌', '月结', '25日', '次月5日', '50000', '12000', '2000', '华联超市有限公司', '9133****', '正常', '编辑 价格 应收 冻结 停用'],
  ...
  supplier: {
    ...
    row: ['G001', '农夫山泉杭州经销', '农夫杭州', '饮料供应商', '赵经理', '0571-8888', '5', '月结', '30', '李四', '工商银行 6222****', '农夫山泉杭州经销有限公司', '9133****', '6600', '正常', '编辑 账户 应付 停用'],
  ...
  warehouse: {
    ...
    row: ['W001', '总仓', '平台货主', '正常仓', '平台主仓', 'CG01', '王五', '杭州市西湖区', '是', '正常', '编辑 停用'],
  ...
  purchaseOrder: {
    ...
    row: ['PO202606140001', '2026-06-14', '平台货主', '农夫山泉杭州经销', '李四', '总仓', '2026-06-18', '月结30天', '3500.00', '0.00', '3120.00', '未付款', '待审核', '未到货', '管理员 10:20', '查看 编辑 保存草稿 审核 反审核 终止 删除 打印 日志'],
  ...
  purchaseInbound: {
    ...
    row: ['PI202606140001', '2026-06-14', 'PO202606140001', '农夫山泉杭州经销', '总仓', 'A01-01', '100', '100', 'B202606', '2026-06-01', '2027-06-01', '30.80', '31.20', '100.00', '3500.00', '待审核', '否', '否', '查看 编辑 审核 打印'],
  },
  quickOrder: {
    ...moduleConfigs.quickOrder,
    columns: ['商品', '仓库', '业务单位', '开单数量', '换算率', '基本单位数量', '可用库存', '单价', '折扣', '金额', '成本', '毛利', '赠品'],
    row: ['农夫山泉500ml*24', '总仓', '箱', '10', '24', '240', '1200', '35.00', '100%', '350.00', '312.00', '38.00', '否'],
  ...
  salesOrder: {
    ...
    row: ['SO202606140001', '2026-06-14', '华联超市', '张三', '总仓', '正常', '350.00', '0.00', '350.00', '312.00', '通过', '通过', '待审核', '未出库', '未签收', '管理员', '查看 编辑 保存草稿 审核 反审核 关闭 删除 打印'],
  ...
  salesOutbound: {
    ...
    row: ['SOU202606140001', 'SO202606140001', '华联超市', '总仓', '2026-06-14', '10', 'B202606', '350.00', '312.00', '未生成', '待审核', '查看 审核 打印'],
  },
  salesReceipt: {
    ...moduleConfigs.salesReceipt,
    actions: ['确认签收', '修改签收单价', '审核', '反审核', '查看应收', '导出', '打印'],
    row: ['SR202606140001', 'SO202606140001', '华联超市', '2026-06-14', '350.00', '未生成', '全部签收', '待审核', '确认签收 修改签收单价 审核 查看应收'],
  },
})
