  ...
  <div class="rpt-page">
      ...
      <div class="rpt-title">
        ...
        <details class="rpt-help">
          ...
          <div class="rpt-help-pop">
            业务员月度回访主表：按客户（必选维度）+ 商品等维度汇总签收/退货/净销售/成本/毛利。
            销售按司机签收日期口径（含税），退货按退货审核日期；拒收不计销售。
            数据来自销售域日汇总（DWS），最多 3 级分组；组小计/合计前端实时汇总，
            毛利率按「组毛利÷组净额」现算。数据范围按客户/业务员强隔离，
            成本/毛利列按角色橘色脱敏。签收/退货列可钻取到「商品销售明细表」。
          </div>
        </details>
      </div>
    ...
    <div class="rpt-group-bar">
      ...
      <select v-for="(_, i) in 3" :key="i" :value="groupBy[i] || ''" @change="onGroupChange(i, $event.target.value)">
        <option value="">{{ i === 0 ? '（默认：客户+商品）' : `第${i + 1}级：不分组` }}</option>
        <option v-for="o in groupOptions(i)" :key="o.key" :value="o.key">{{ o.title }}</option>
      </select>
      <span class="rpt-group-tip">必须包含「客户」；最多 3 级</span>
    </div>
...
const MODULE = 'customerGoodsSummaryReport'
const CODE = 'customer_goods_summary'
const STORE_KEY = 'rpt:customer_goods_summary:state'
...
// 可选分组维度（白名单与后端 groupWhitelist 严格一致；后端强制必须含 customer）
const GROUP_META = [
  ...
  { key: 'customerLevel', title: '客户等级', outputKey: 'customerLevel' },
...
onMounted(async () => {
  ...
  if (groupBy.value.length === 0) groupBy.value = ['customer', 'goods']
  await r.query()
})
...
function applyDrillQuery() {
  ...
  drillBanner.value = {
    from: q.from || '',
    fromName: FROM_NAME[q.from] || '客户销售汇总表',
    text: texts.join('；'),
  }
}
...
function onGroupChange(i, val) {
  ...
  // 前端先拦一道，避免请求被后端拒绝（钻取 #16 依赖客户维度）
  if (!next.includes('customer')) {
    alert('客户商品销售汇总必须包含「客户」分组维度')
    return
  }
  groupBy.value = next
  pageNo.value = 1
...
const visibleColumns = computed(() => {
  ...
  for (const g of groups) {
    if (g === 'customer') {
      cols.push({ key: 'customerCode', title: '客户编号', width: 100, sortable: true })
      cols.push({ key: 'customerName', title: '客户名称', width: 160 })
    } else if (g === 'territory') cols.push({ key: 'territory', title: '区域', width: 90 })
    else if (g === 'salesman') cols.push({ key: 'salesman', title: '业务员', width: 90 })
    else if (g === 'customerLevel') cols.push({ key: 'customerLevel', title: '客户等级', width: 90 })
  ...
  // 客户叶子附带的客户属性（该属性已作为分组层级时不重复）
  if (groups.includes('customer')) {
    if (!groups.includes('customerLevel')) cols.push({ key: 'customerLevel', title: '客户等级', width: 90 })
    if (!groups.includes('territory')) cols.push({ key: 'territory', title: '区域', width: 90 })
    cols.push({ key: 'routeLine', title: '路线', width: 90 })
...
/** 钻取到 #16：客户+商品+期间+类型；维度值回带，筛选条条件不放宽 */
function onDrill({ group, dims, row, col }) {
  ...
  if (src.customerName) q.customer = src.customerName
  if (src.territory) q.territory = src.territory
  if (src.salesman) q.salesman = src.salesman
  if (src.customerLevel) q.customerLevel = src.customerLevel
