  ...
  <div class="rpt-page">
      ...
      <div class="rpt-title">
        ...
        <details class="rpt-help">
          ...
          <div class="rpt-help-pop">
            数据来自采购域日汇总（DWS，每日日结后刷新），按所选维度（最多 3 级）聚合；
            各组小计与合计为前端对叶子行实时汇总，与明细同口径。入库/退货为发生额（退货列正数展示），
            采购数量/金额=入库−退货轧抵净额。蓝色数字可钻取到「采购明细查询」逐笔核对（最多返回 10 万行叶子）。
          </div>
        </details>
      </div>
    ...
    <div class="rpt-group-bar">
      ...
      <select v-for="(_, i) in 3" :key="i" :value="groupBy[i] || ''" @change="onGroupChange(i, $event.target.value)">
        <option value="">{{ i === 0 ? '（默认：商品+采购员）' : `第${i + 1}级：不分组` }}</option>
...
const MODULE = 'purchaseGoodsSummaryReport'
const CODE = 'purchase_goods_summary'
const STORE_KEY = 'rpt:purchase_goods_summary:state'
...
onMounted(async () => {
  ...
  rememberLoad()
  if (groupBy.value.length === 0) groupBy.value = ['goods', 'buyer']
  await r.query()
})
...
const visibleColumns = computed(() => {
  ...
  for (const g of groups) {
    ...
    else if (g === 'buyer') cols.push({ key: 'buyer', title: '采购员', width: 90 })
