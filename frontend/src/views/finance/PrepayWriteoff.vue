 ...
 * 结算自动单（AP_SETTLE/STATEMENT_SETTLE）：只可查看；冲回入口是对应付款单反审核（级联作废）。
 */
import { onMounted, ref } from 'vue'
import { post } from '../../api/client.js'
import PrepayWriteoffDrawer from '../../components/PrepayWriteoffDrawer.vue'
...
async function load() {
  ...
  feedback.value = null
  try {
    const res = await post('/finance/prepay-writeoff/page', {
      pageNo: pageNo.value,
...
async function doAudit(row) {
  if (!window.confirm(`确认审核预付核销单 ${row.writeoffNo}？\n审核后预付冲应付即刻生效。`)) return
  acting.value = true
  feedback.value = null
  try {
    await post('/finance/prepay-writeoff/audit', { writeoffId: row.writeoffId })
...
async function doCancelAudit(row) {
  if (!window.confirm(`确认反审核预付核销单 ${row.writeoffNo}？\n核销将全部冲回，预付余额与应付金额恢复。`)) return
  acting.value = true
  feedback.value = null
  try {
    await post('/finance/prepay-writeoff/cancel-audit', { writeoffId: row.writeoffId })
...
async function doDelete(row) {
  if (!window.confirm(`确认删除预付核销单 ${row.writeoffNo}？仅待审核单据可删除。`)) return
  acting.value = true
  feedback.value = null
  try {
    await post('/finance/prepay-writeoff/delete', { writeoffId: row.writeoffId })
...
<template>
  <div class="module-body">
    <div class="page-ops">
      ...
      <button class="btn" v-permission="'fin.prepay_writeoff.add'" @click="openCreate">新建核销单</button>
      ...
      <div class="query-row">
        ...
        <label>业务来源
          <select v-model="filters.businessSource" class="sel">
            ...
            <option value="AP_SETTLE">应付结算</option>
          ...
          <tr v-for="(r, i) in records" :key="r.writeoffId">
            ...
            <td class="ops">
              ...
              <template v-if="isManual(r) && r.status === 'PENDING'">
                <a class="lk" v-permission="'fin.prepay_writeoff.edit'" @click="openEdit(r)">编辑</a>
                <a class="lk danger" v-permission="'fin.prepay_writeoff.delete'"
                   @click="doDelete(r)">删除</a>
                <a class="lk primary" v-permission="'fin.prepay_writeoff.audit'"
                   @click="doAudit(r)">审核</a>
              </template>
              <a v-else-if="isManual(r) && r.status === 'APPROVED'" class="lk warn"
                 v-permission="'fin.prepay_writeoff.unaudit'" @click="doCancelAudit(r)">反审核</a>
    ...
    <PrepayWriteoffDrawer ref="drawer" @saved="load" />
  </div>
</template>
