...
// 本抽屉仅被财务费用单（financeExpense → fin.fee）使用；金额/税额/不含税走资金视角字段权限
const { canView, guardCode } = useRbac('financeExpense')
...
// 保存功能点：新建 fin.fee.add，编辑 fin.fee.edit
const savePermCode = computed(() => `fin.fee.${props.mode === 'edit' ? 'edit' : 'add'}`)
