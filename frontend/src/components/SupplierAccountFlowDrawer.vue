<script setup>
/**
 * 供应商往来流水查询（PRD-36 M1）：应付/预付/费用三本账。
 * 从供应商账户列表点余额打开；条件仅点「查询」生效；展示此前余额与滚存账户金额。
 */
...
const supplier = ref({ supplierCode: '', supplierName: '' })
const accountType = ref('AP')
const records = ref([])
const total = ref(0)
const pageNo = ref(1)
...
const ACCOUNT_TABS = [
  { type: 'AP', label: '应付' },
  { type: 'PREPAY', label: '预付' },
  { type: 'EXPENSE', label: '费用' },
]
const tabLabel = computed(() => ACCOUNT_TABS.find(t => t.type === accountType.value)?.label || '')
const statusOptions = computed(() => {
  if (accountType.value === 'EXPENSE') {
    return ['待兑现', '部分兑现', '已兑现']
  }
  if (accountType.value === 'AP') {
    return ['未结算', '部分结算', '已结算']
  }
  return []
})
...
async function load() {
  ...
  feedback.value = null
  try {
    const res = await post('/finance/supplier-account/flow/page', {
...
<template>
  <div v-if="visible" class="flow-mask" @click.self="close">
    <div class="flow-modal">
      <div class="flow-h">
        往来流水查询 [{{ supplier.supplierName }}]
        <span class="x" @click="close">×</span>
      </div>

      <div class="flow-b">
        <div class="tabs">
          <button v-for="t in ACCOUNT_TABS" :key="t.type" class="tab"
                  :class="{ on: accountType === t.type }" @click="switchTab(t.type)">{{ t.label }}</button>
        </div>
          ...
          <table class="flow-table">
            <thead>
              <tr>
                ...
                <th style="width:110px">业务单据</th>
                <th style="width:110px">增加金额</th>
                <th style="width:110px">减少金额</th>
                <th style="width:110px">账户金额</th>
                <th style="width:80px">{{ accountType === 'EXPENSE' ? '兑现标志' : '结算标志' }}</th>
                <th>摘要</th>
                <th style="width:150px">{{ accountType === 'EXPENSE' ? '厂家费用单号' : '应付单号' }}</th>
                <th style="width:160px">来源单据号</th>
                <th style="width:150px">结算/兑现单号</th>
              </tr>
            </thead>
            <tbody>
              ...
              <tr v-for="(r, i) in records" :key="r.flowId" :class="{ redrow: r.isRed === 'Y' }">
                ...
                <td class="bill">{{ r.factoryExpenseNo || r.apNo }}</td>
                <td class="bill">{{ r.sourceBill }}</td>
                <td class="bill">{{ r.writeoffNo || r.paymentNo }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
.settle.未结算, .settle.待兑现 { background: #f4f4f5; color: #909399; }
.settle.部分结算, .settle.部分兑现 { background: #fdf6ec; color: #b88230; }
.settle.已结算, .settle.已兑现 { background: #f0f9eb; color: #2f8f46; }
