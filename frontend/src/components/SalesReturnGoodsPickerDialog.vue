<script setup>
/**
 * 销售退货 —— 商品档案选择窗口（【添加商品】入口）
 *
 * 两个页签：
 *   · 历史销售（默认）：该客户已审核出库单里出现过的商品，按最近销售日期倒序
 *   · 全部商品：全部正常商品
 *
 * 交互约定：
...
async function loadGoods() {
  ...
  try {
    ...
    if (props.customerName) params.set('customerName', props.customerName)
    if (props.warehouse) params.set('warehouse', props.warehouse)
    if (keyword.value.trim()) params.set('keyword', keyword.value.trim())
    const rows = await get(`/sales/return-order/goods-options?${params.toString()}`)
    goodsList.value = (Array.isArray(rows) ? rows : []).map(g => ({
      ...g,
      latestSalePrice: Number(g.latestSalePrice || 0),
...
function confirmSelection(close = false) {
  ...
  const payload = rows.map(g => ({
    ...
    qty: 0,                                                   // 按品退货数量由用户填
    price: g.latestSalePrice || g.costPrice || 0,              // 参考售价兜底成本价
    batchNo: '',
    productionDate: '',
    sourceOutboundNo: '',
    sourceDetailId: '',
...
<template>
  <div v-if="visible" class="gpd-mask" @click.self="closeDialog">
    <div class="gpd-box">
      <div class="gpd-head">
        <b>添加商品 —— 按品退货</b>
        <span class="gpd-sup-tag">客户：{{ customerName || '未选择' }}</span>
        ...
        <table v-else>
          <thead>
            <tr>
              ...
              <th style="width:95px">参考售价</th>
              <th style="width:100px">成本单价</th>
              <th style="width:90px">可用库存</th>
              <th v-if="activeTab === 'HISTORY'" style="width:110px">最近出库日期</th>
            </tr>
          </thead>
          <tbody>
            <tr
            ...
            >
              ...
              <td>{{ g.spec || '-' }}</td>
              <td>{{ g.baseUnit || '-' }}</td>
              <td class="num">{{ (Number(g.latestSalePrice || 0)).toFixed(4) }}</td>
              <td class="num">{{ (Number(g.costPrice || 0)).toFixed(6) }}</td>
              <td class="num" :class="{ zero: Number(g.availableStock || 0) <= 0 }">{{ g.availableStock }}</td>
              <td v-if="activeTab === 'HISTORY'">{{ String(g.lastOutboundDate || '').slice(0, 10) || '-' }}</td>
            </tr>
          </tbody>
        </table>
      </div>
