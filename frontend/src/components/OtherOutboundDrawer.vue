<script setup>
/**
 * 其他出库单抽屉 —— 新建 / 编辑 / 查看其他出库单。
 *
 * 用途：非销售、非采购退货、非报损且不形成应收的库存减少（内部领用/样品出库/借出出库/活动消耗等）。
 *
 * 结构参考报损单抽屉（DamageDrawer）：明细通过【添加商品】从当前仓库的批次库存中选取，
 * 一行 = 一个批次，出库数量不可超过该批次可用库存。
 * 主表字段参考其他入库单（OtherInboundDrawer）：单据日期、客户/供应商二选一、
 * 其他出库类型（取字典 other_outbound_type）、仓库、备注。
 *
 * 流程：新建（保存即为「未审核」，无草稿态）→ 编辑 → 审核（扣库存）→ 取消审核（回库）。
 * 支持作废和删除（仅未审核）。
 */
...
const props = defineProps({
  ...
  /** 编辑/查看已有单据时传入 { outboundId, outboundNo } */
...
/** 其他出库类型字典编码，与 V55 迁移里的 dict_type 一致 */
const DICT_OTHER_OUTBOUND_TYPE = 'other_outbound_type'
...
const isEdit = computed(() => !!props.editData?.outboundId)

const canEdit = computed(() => {
  if (props.readonly) return false
  if (!isEdit.value) return true
  return outboundStatus.value === 'DRAFT' || outboundStatus.value === 'PENDING'
})

const readonlyReason = computed(() => {
  if (canEdit.value) return ''
  switch (outboundStatus.value) {
    case 'APPROVED': return '该其他出库单已审核，只可查看。如需修改请先取消审核。'
    case 'CANCELLED': return '该其他出库单已作废，只可查看。'
    default: return '该其他出库单当前状态不可编辑，只可查看。'
  }
})
...
const statusText = computed(() => ({
  ...
  APPROVED: '已审核',
  CANCELLED: '已作废',
}[outboundStatus.value] || outboundStatus.value))
...
watch(() => props.visible, async (val) => {
  ...
  if (props.editData?.outboundId) {
    await loadExisting(props.editData.outboundId)
  }
})
...
async function loadBaseOptions() {
  ...
  try {
    const [wh, cust, supp, dict] = await Promise.all([
      ...
      getDict(DICT_OTHER_OUTBOUND_TYPE).catch(() => []),
    ])
    ...
    outboundTypeOptions.value = (dict || []).map(d => d.name).filter(Boolean)
  } catch {
...
async function loadExisting(outboundId) {
  loading.value = true
  try {
    const data = await get(`/inventory/other-outbound/detail?outboundId=${encodeURIComponent(outboundId)}`)
    outboundStatus.value = data.status || ''
    headerForm.value = {
    ...
    errors.value.header = e.message || '加载其他出库单失败'
  } finally {
    loading.value = false
  }
}
...
async function loadGoodsPickerList() {
  ...
  try {
    ...
    const rows = await get(`/inventory/other-outbound/batch-stock-options?${params.toString()}`)
...
function confirmGoodsSelection() {
  ...
  if (selected.length === 0) {
    goodsPickerError.value = '请先勾选要出库的批次'
    return
  }
  ...
  for (const r of selected) {
    ...
    detailList.value.push({
      ...
      unitName: r.baseUnit || '',
      qty: 0,
      // 单价默认取标准售价，无标准售价时回落到成本单价
      price: Number(r.standardPrice || 0) > 0 ? Number(r.standardPrice) : Number(r.costPrice || 0),
      amount: 0,
      batchNo: r.batchNo || '',
...
async function openBatchPicker(index) {
  ...
  try {
    const rows = await get(
      `/inventory/other-outbound/batch-options?goodsCode=${encodeURIComponent(row.goodsCode)}&warehouse=${encodeURIComponent(headerForm.value.warehouse)}`
    )
    batchCache.value = {
      ...batchCache.value,
...
function onQtyChange(index) {
  ...
  if (row.availableStock > 0 && q > row.availableStock) {
    errors.value.details = `商品 ${row.goodsName} 出库数量 ${q} 超过可用库存 ${row.availableStock}`
  } else {
...
function validate() {
  errors.value = {}
  const { customer, supplier } = headerForm.value
  if (customer && supplier) { errors.value.header = '客户与供应商只能选择一个'; return false }
  if (!customer && !supplier) { errors.value.header = '请选择客户或供应商'; return false }
  if (!headerForm.value.outboundType) { errors.value.header = '请选择其他出库类型'; return false }
  if (!headerForm.value.warehouse) { errors.value.header = '请选择仓库'; return false }
  if (detailList.value.length === 0) { errors.value.details = '请添加出库商品'; return false }
  for (let i = 0; i < detailList.value.length; i++) {
    ...
    if (!r.qty || Number(r.qty) <= 0) { errors.value.details = `商品 ${r.goodsName} 的出库数量必须大于 0`; return false }
    if (Number(r.availableStock || 0) > 0 && Number(r.qty) > Number(r.availableStock)) {
      errors.value.details = `商品 ${r.goodsName} 出库数量 ${r.qty} 超过可用库存 ${r.availableStock}`
      return false
    }
  }
  const seen = new Map()
...
async function saveOutbound() {
  ...
  try {
    const payload = {
      ...
      outboundType: headerForm.value.outboundType,
    ...
    if (isEdit.value) {
      payload.outboundId = props.editData.outboundId
      await post('/inventory/other-outbound/update', payload)
    } else {
      await post('/inventory/other-outbound/create', payload)
    }
...
async function handleAudit() {
  ...
  loading.value = true
  try {
    const res = await post('/inventory/other-outbound/audit', { bizId: props.editData.outboundId })
    outboundStatus.value = 'APPROVED'
    errors.value = {}
...
async function handleReverseAudit() {
  ...
  loading.value = true
  try {
    const res = await post('/inventory/other-outbound/reverse-audit', { bizId: props.editData.outboundId })
    outboundStatus.value = 'PENDING'
    errors.value = {}
    alert(res?.effect || '取消审核成功')
    emit('save', { status: 'PENDING' })
  } catch (e) {
    errors.value.header = e.message || '取消审核失败'
  } finally {
    loading.value = false
  }
}
...
async function handleCancel() {
  if (!confirm('确认作废该其他出库单？此操作不可逆，已审核的将自动回滚库存。')) return
  loading.value = true
  try {
    const res = await post('/inventory/other-outbound/cancel', { bizId: props.editData.outboundId })
    outboundStatus.value = 'CANCELLED'
    errors.value = {}
...
<template>
  <div v-if="visible" class="oob-drawer-mask">
    <div class="oob-drawer-box">
      <!-- 头部 -->
      <div class="oob-drawer-head">
        <b>{{ isEdit ? '编辑其他出库单' : '新建其他出库单' }}</b>
        <span v-if="isEdit" class="status-tag" :class="'st-' + outboundStatus.toLowerCase()">{{ statusText }}</span>
        <span style="flex:1"></span>
        <button v-if="canEdit" class="btn primary" @click="saveOutbound()" :disabled="loading">保存</button>
        <button v-if="isEdit && (outboundStatus === 'PENDING' || outboundStatus === 'DRAFT')" class="btn primary" @click="handleAudit" :disabled="loading">审核</button>
        <button v-if="isEdit && outboundStatus === 'APPROVED'" class="btn" @click="handleReverseAudit" :disabled="loading">取消审核</button>
        <button v-if="isEdit && outboundStatus !== 'CANCELLED'" class="btn danger" @click="handleCancel" :disabled="loading">作废</button>
        <button class="btn" @click="handleClose">关闭</button>
      </div>

      <!-- 主体 -->
      <div class="oob-drawer-body">
            ...
            <table>
              ...
              <tbody>
                <tr v-if="detailList.length === 0">
                  <td colspan="14" class="empty-detail">暂无出库商品，请点击"添加商品"</td>
                </tr>
...
.oob-drawer-head b { font-size: 15px; }
.status-tag {
...
.field .hint { color: #bbb; font-weight: 400; }
.field input, .field select {
  padding: 5px 8px; border: 1px solid #d9d9d9; border-radius: 4px; font-size: 13px;
}
.field input:disabled { background: #f5f7fa; color: #c0c4cc; }
