<script setup>
/**
 * 销售出库单商品选择窗口（【按单添加商品】入口）
 *
 * 左右表格结构：
 *   · 左表：该客户已审核的销售出库单（出库单号、单据日期）
...
const billQuery = ref({ outboundNo: '', dateFrom: '', dateTo: '' })
...
const selectedBill = computed(() => billList.value.find(b => b.outboundId === selectedBillId.value) || null)
/** 可勾选行 = 可退数量 > 0 */
...
watch(() => props.visible, async (val) => {
  ...
  detailList.value = []
  goodsKeyword.value = ''
  billQuery.value = { outboundNo: '', dateFrom: oneYearAgo(), dateTo: today() }
  await loadBills()
})
...
async function loadBills() {
  ...
  try {
    ...
    if (billQuery.value.outboundNo) params.set('outboundNo', billQuery.value.outboundNo)
    const rows = await get(`/sales/return-order/outbound-bills?${params.toString()}`)
    billList.value = Array.isArray(rows) ? rows : []
    // 默认选中第一行
    if (billList.value.length > 0) {
      await selectBill(billList.value[0].outboundId)
    } else {
      selectedBillId.value = ''
      detailList.value = []
    }
  } catch (e) {
    errorMsg.value = e.message || '加载出库单失败'
    billList.value = []
    detailList.value = []
  } finally {
...
async function selectBill(outboundId) {
  if (!outboundId) return
  selectedBillId.value = outboundId
  await loadDetails()
}
...
async function loadDetails() {
  ...
  try {
    const params = new URLSearchParams({ outboundId: selectedBillId.value })
    if (goodsKeyword.value.trim()) params.set('goodsKeyword', goodsKeyword.value.trim())
    const data = await get(`/sales/return-order/outbound-detail?${params.toString()}`)
    detailList.value = (data.details || []).map(d => ({
      ...
      sourceOutboundNo: d.sourceOutboundNo || data.outboundNo,
      warehouse: data.warehouse,
    }))
  } catch (e) {
    errorMsg.value = e.message || '加载出库明细失败'
    detailList.value = []
  } finally {
...
<template>
  <div v-if="visible" class="opd-mask" @click.self="closeDialog">
    <div class="opd-box">
      <div class="opd-head">
        <b>按单添加商品 —— 选择销售出库单</b>
        <span class="opd-sup-tag">客户：{{ customerName || '未选择' }}</span>
      ...
      <div v-if="errorMsg" class="opd-err">{{ errorMsg }}</div>

      <div class="opd-body">
        <!-- 左：出库单列表 -->
        <div class="opd-left">
          <div class="opd-panel-title">已审核出库单</div>
          <div class="opd-filter">
            <input v-model="billQuery.outboundNo" placeholder="出库单号" @keydown.enter="loadBills" />
            <div class="opd-date-row">
          ...
          <div class="opd-table-wrap">
            <div v-if="billLoading" class="opd-empty">加载中...</div>
            <div v-else-if="billList.length === 0" class="opd-empty">无符合条件的已审核出库单</div>
            <table v-else>
              <thead>
                <tr>
                  <th>出库单号</th>
              ...
              <tbody>
                <tr
                  ...
                  :key="b.outboundId"
                  class="opd-bill-row"
                  :class="{ active: b.outboundId === selectedBillId }"
                  @click="selectBill(b.outboundId)"
                >
                  <td>{{ b.outboundNo }}</td>
        ...
        <!-- 右：出库明细 -->
        <div class="opd-right">
          <div class="opd-panel-title">
            出库明细
            <span v-if="selectedBill" class="opd-sub">{{ selectedBill.outboundNo }} · {{ selectedBill.warehouse }}</span>
          </div>
          <div class="opd-filter">
          ...
          <div class="opd-table-wrap">
            <div v-if="detailLoading" class="opd-empty">加载中...</div>
            <div v-else-if="!selectedBillId" class="opd-empty">请先在左侧选择出库单</div>
            <div v-else-if="detailList.length === 0" class="opd-empty">该单据无匹配明细</div>
            <table v-else>
              <thead>
                <tr>
                  <th style="width:36px">
...
.opd-head b { font-size: 14px; }
.opd-sup { color: #666; font-size: 12px; }
/* 客户标签：顶部醒目展示当前所选客户 */
.opd-sup-tag {
  padding: 3px 10px;
  background: #ecf5ff;
...
.opd-left { width: 38%; min-width: 0; display: flex; flex-direction: column; }
.opd-right { flex: 1; min-width: 0; display: flex; flex-direction: column; }
.opd-panel-title {
  font-weight: 700; font-size: 13px; color: var(--primary, #409eff);
  padding-bottom: 6px;
  display: flex; align-items: center; gap: 8px;
}
.opd-sub { font-weight: 400; font-size: 12px; color: #909399; }
.opd-filter {
...
.opd-filter input[type="date"] { flex: none; width: 130px; }
.opd-date-row { display: flex; align-items: center; gap: 4px; }
.opd-date-row .sep { color: #909399; font-size: 12px; }
.opd-table-wrap {
  flex: 1; min-height: 0; overflow: auto;
  border: 1px solid #e5e7eb; border-radius: 6px;
}
.opd-table-wrap table { width: 100%; border-collapse: collapse; font-size: 12px; }
.opd-table-wrap th {
...
.opd-table-wrap td.num { text-align: right; font-variant-numeric: tabular-nums; }
.opd-table-wrap td.num.warn { color: #e6a23c; font-weight: 700; }
.opd-bill-row { cursor: pointer; }
.opd-bill-row:hover { background: #f5f7fa; }
.opd-bill-row.active { background: #ecf5ff; font-weight: 700; }
.opd-table-wrap tr.disabled { color: #c0c4cc; background: #fafafa; }
.opd-table-wrap tr.checked { background: #f0f9eb; }
.opd-empty { padding: 40px; text-align: center; color: #909399; font-size: 12px; }
.opd-foot {
...
.opd-foot .ok { color: #67c23a; }
.opd-foot .tip { color: #909399; }
</style>
